...
document.addEventListener("DOMContentLoaded", ()=>{
    ...
    async function renderPoke(page = 1) {
        try {
            ...
            console.error("Error rendering Pok\xe9mon cards:", error);
        }
    }
    function renderPokemonCard(pokemon) {
        ...
        // Get all type names
        const typeNames = pokemon.types.map((typeInfo)=>typeInfo.type.name).join(", ");
        const cardContent = `
        <div class="card">
    ...
    async function searchPoke(event) {
        ...
        try {
            ...
            const matchingPokemon = allPokemons.find((pokemon)=>pokemon.name.toLowerCase() === searchinput);
            pokemonContainer.innerHTML = ""; // Clear existing cards
            if (matchingPokemon) {
                const specificPoke = await fetchSpecificApi(matchingPokemon.url);
                renderPokemonCard(specificPoke);
            } else {
                console.log("No matching Pok\xe9mon found.");
                renderPoke(currentPage);
            }
        } catch (error) {
            console.error("Error searching Pok\xe9mon:", error);
        }
    }
    function renderPagination() {
      ...
      `;
        ...
        if (endPage - startPage + 1 < maxPagesToShow) startPage = Math.max(endPage - maxPagesToShow + 1, 1);
    ...
    async function showModal(pokemonId) {
        const pokemon = await fetchSpecificApi(`https://pokeapi.co/api/v2/pokemon/${pokemonId}`);
        if (pokemon) {
            ...
            document.getElementById("modalPokemonTypes").textContent = `Type: ${pokemon.types.map((typeInfo)=>typeInfo.type.name).join(", ")}`;
            document.getElementById("modalPokemonHeight").textContent = `Height: ${pokemon.height}`;
            document.getElementById("modalPokemonWeight").textContent = `Weight: ${pokemon.weight}`;
            document.getElementById("modalPokemonAbilities").textContent = `Abilities: ${pokemon.abilities.map((abilityInfo)=>abilityInfo.ability.name).join(", ")}`;
        }
    }
    document.querySelector("#pokecontainer").addEventListener("click", (event)=>{
        ...
        if (button) {
            ...
            if (pokemonId) showModal(pokemonId);
        }
    });
...
},{"./createPopper.js":"cHuNp","./modifiers/eventListeners.js":"hBKsL","./modifiers/popperOffsets.js":"6I679","./modifiers/computeStyles.js":"gDlm2","./modifiers/applyStyles.js":"4iMn4","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}]},["j2YDk","1SICI"], "1SICI", "parcelRequire561b")
